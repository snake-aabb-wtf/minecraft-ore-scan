...
所以服务端目录是 <程序目录>/Minecraft，Excel 输出目录是 <程序目录>，不是当前 PowerShell 的任意工作目录。current_server_dir 指向用户已明确选中的具体版本目录。主界面的服务端管理窗口可以切换、安装或卸载版本；安装向导仅在发现已有版本时显示“管理已有”入口。

启动时如果 Minecraft 下存在任何包含 server.jar 的子目录，就显示“已有服务端选择向导”，在列表中展示全部已发现版本。不得自动选取发现列表中的第一个服务端；current_server_dir 保持为空，直到用户选择一个版本并点击“使用选中”。该向导提供“安装新版本”和“卸载选中”入口；卸载必须先确认，成功后刷新选择页，最后一个版本被删除时进入安装页。没有已安装服务端时才显示安装页。服务端管理页可以在已发现的目录之间切换，或回到安装页安装新版本，并复用相同的确认卸载逻辑。主扫描页会异步读取当前服务端的 Mojang Java 要求、检测本机 Java，并显示可用性状态；扫描开始前必须再次执行相同检查。缺少精确匹配版本时状态文字必须为红色，并显示打开 Adoptium Temurin 发行版页的安装按钮。
...
- custom_ore_id：自定义方块 ID，需先勾选"自定义方块"复选框才会启用并生效。勾选后填写必须符合 Minecraft 资源定位符语法（namespace:path，仅小写字母/数字/下划线/点/连字符/斜杠，长度不超过 128），校验通过后追加进目标集合；勾选但为空或格式非法都会阻止扫描并提示，未勾选则输入框禁用且完全不参与扫描。用于扫描模组方块或末地等无内置选项的维度；Excel 中显示原始 block ID。
- min_free_gb：最低剩余磁盘空间，空白或非正数表示不限制；默认值为 8 GB。

矿物列表由 constants.py 按维度切换。主世界默认选择钻石矿和深板岩钻石矿；下界默认选择远古残骸、下界石英矿和下界金矿；末地当前没有可选矿物，只能通过勾选自定义方块并填写 ID 扫描。"自定义方块"复选框与 ID 输入框放在一个 columnspan=5 的子容器中、grid 在矿物复选框网格的下一行（不参与矿物列的列宽计算——若直接放入矿物列，width=30 的输入框会把该列撑宽，造成相邻矿物复选框之间的异常间距），随维度切换重建但保留勾选状态与已输入值。
...
- validate_custom_block_id(text)：空字符串合法（表示不指定）；拒绝非字符串、超长（>128）、换行/等号和不满足资源定位符语法（`^[a-z0-9_.-]+:[a-z0-9/._-]+$`）的输入，返回清洗后的 block ID。
...
- 修改启动服务端选择流程时，验证已安装服务端不会被自动选中、选择页列出全部版本、“使用选中”会进入扫描页，以及空列表仍进入安装向导。
