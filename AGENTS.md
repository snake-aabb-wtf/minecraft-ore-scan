...
矿物列表由 constants.py 按维度切换。主世界默认选择钻石矿和深板岩钻石矿；下界默认选择远古残骸、下界石英矿和下界金矿；末地当前没有可选矿物，只能通过勾选自定义方块并填写 ID 扫描。"自定义方块"复选框与 ID 输入框位于矿物复选框右侧（主世界即"深层绿宝石矿"旁边），随维度切换重建但保留勾选状态与已输入值。
