...
- custom_ore_id：自定义方块 ID（空字符串表示不指定）。必须是 Minecraft 资源定位符语法（namespace:path，仅小写字母/数字/下划线/点/连字符/斜杠，长度不超过 128），校验通过后追加进目标集合，用于扫描模组方块或末地等无内置选项的维度；Excel 中显示原始 block ID。
- min_free_gb：最低剩余磁盘空间，空白或非正数表示不限制；默认值为 8 GB。

矿物列表由 constants.py 按维度切换。主世界默认选择钻石矿和深板岩钻石矿；下界默认选择远古残骸、下界石英矿和下界金矿；末地当前没有可选矿物，只能通过自定义方块 ID 扫描。自定义 ID 输入框位于矿物复选框右侧（主世界即"深层绿宝石矿"旁边），随维度切换重建但保留已输入值。
