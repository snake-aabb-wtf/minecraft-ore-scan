...
启动时如果 Minecraft/ 下已有包含 server.jar 的版本目录，程序会先显示“已有服务端选择向导”，列出全部已安装版本。必须选择一个版本并点击“使用选中”后才会进入扫描界面，程序不会自动使用发现列表中的第一个服务端；该页面也提供“安装新版本”入口。主扫描界面的“管理服务端”窗口可以切换已安装版本、打开安装向导或卸载选中版本。服务端安装向导在检测到已有版本时会显示“管理已有”按钮，可返回该窗口。卸载会删除该版本的整个目录，包括 server.jar、世界数据、世界备份和服务端配置；程序会在确认后执行。卸载当前最后一个服务端后，GUI 自动返回服务端安装向导，且不显示“管理已有”按钮。
...
启动时，如果 Minecraft/ 下已有包含 server.jar 的版本目录，程序会显示“已有服务端选择向导”。列表列出全部已安装版本；选择其中一个版本并点击“使用选中”后，才会进入扫描界面。用户也可以从该页面点击“安装新版本”。如果没有任何包含 server.jar 的版本目录，程序会直接显示服务端安装向导。
