...
主扫描界面的“管理服务端”窗口可以切换已安装版本、打开安装向导或卸载选中版本。服务端安装向导在检测到已有版本时会显示“管理已有”按钮，可返回该窗口。卸载会删除该版本的整个目录，包括 server.jar、世界数据、世界备份和服务端配置；程序会在确认后执行。卸载当前最后一个服务端后，GUI 自动返回服务端安装向导，且不显示“管理已有”按钮。
