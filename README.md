...
启动时如果 Minecraft/ 下已有包含 server.jar 的版本目录，程序会先显示“已有服务端选择向导”，列出全部已安装版本。必须选择一个版本并点击“使用选中”后才会进入扫描界面，程序不会自动使用发现列表中的第一个服务端；该页面也提供“安装新版本”和“卸载选中”入口。卸载会先要求确认，随后删除该版本的整个目录，包括 server.jar、世界数据、世界备份和服务端配置；卸载后仍有版本时刷新选择列表，卸载最后一个版本时进入安装向导。主扫描界面的“管理服务端”窗口同样可以切换已安装版本、打开安装向导或卸载选中版本。服务端安装向导在检测到已有版本时会显示“管理已有”按钮，可返回该窗口。
...
启动时，如果 Minecraft/ 下已有包含 server.jar 的版本目录，程序会显示“已有服务端选择向导”。列表列出全部已安装版本；选择其中一个版本并点击“使用选中”后，才会进入扫描界面。用户也可以从该页面点击“安装新版本”或“卸载选中”；卸载需要确认，且删除最后一个服务端后会进入安装向导。如果没有任何包含 server.jar 的版本目录，程序会直接显示服务端安装向导。
